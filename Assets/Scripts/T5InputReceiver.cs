...
public class T5InputReceiver : MonoBehaviour
{
    ...
    public UnityEvent OnSystemPressed { get => _onSystemPressed; set => _onSystemPressed = value; }
    public UnityEvent OnSystemHeld { get => _onSystemHeld; set => _onSystemHeld = value; }
    public UnityEvent OnSystemLifted { get => _onSystemLifted; set => _onSystemLifted = value; }
    ...
    private void Update()
    {
        ...
        if (Input.TryGetTrigger(out float triggerValue))
        {
        ...
        if (Input.TryGetStickTilt(out Vector2 stickTilt) && stickTilt.magnitude > _stickMovementDeadzone)
        {
            _movingStick = true;
